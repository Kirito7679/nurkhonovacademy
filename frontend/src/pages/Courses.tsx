...
import { BookOpen, Lock, CheckCircle, ArrowRight, Eye, Search, Filter, X } from 'lucide-react';
...
const Courses = memo(function Courses() {
  ...
  // Format price with spaces and UZS
  const formatPrice = (price: number | null | undefined): string => {
    if (!price || price === 0) return 'Бесплатно';
    return `${price.toLocaleString('ru-RU')} UZS`;
  };

  // Get course price (prioritize subscription prices, fallback to base price)
  const getCoursePrice = (course: Course): number | null => {
    if (course.subscriptionType === 'FREE') return 0;
    // For PAID courses, show the base price or first available subscription price
    if (course.price30Days) return course.price30Days;
    if (course.price3Months) return course.price3Months;
    if (course.price6Months) return course.price6Months;
    if (course.price1Year) return course.price1Year;
            ...
            >
              ...
              <div className="p-5 md:p-6 lg:p-7">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-neutral-100">
                  ...
                  <div className="text-sm md:text-base font-semibold text-primary-600">
                    {formatPrice(getCoursePrice(course))}
                  </div>
                </div>
              </div>
            </Link>
